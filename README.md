...
- Spielerprofil: Detailansicht mit allen verfügbaren Feldern, in thematische Abschnitte gebündelt statt einer langen flachen Liste - **Basis** (Position, Alter, Vertrag, Gehalt, Marktwert, Einsatzstatus), **Leistung** (Durchschnittsnote, Einsätze, echte Saison-Kennzahlen), **Attribute** (alle glatten 1-20-Werte - die klassische FM-Attributskala), **Weitere Felder** (alles Unklassifizierte, eingeklappt, damit keine Daten verschwinden). Prozentwerte (erkannt am Spaltennamen: "Anteil"/"Prozent"/"-quote"/"(%)") und 1-20-Attribute bekommen zusätzlich einen kompakten Balken - dieselbe Optik wie der Spielervergleich-Chart, zu zweit nebeneinander, wenn die Breite reicht (die Profilseite ist auf 980px gewachsen, um das auszunutzen). Läuft eigenständig (wie Position/Vergleich), daher eine lokale, konservative Erkennung statt geratener FM-Kategorien (z. B. "Technik"/"Mental"/"Physis") - Länderspiel-Zähler wie "Lsp Eins" werden bewusst nicht als Attribut behandelt, obwohl sie zufällig im 1-20-Bereich liegen können
