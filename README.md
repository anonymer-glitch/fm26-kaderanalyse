...
- **Design:** ein Satz CSS-Variablen in `css/styles.css` (`:root`) statt verstreuter Hex-Werte - Farben, Radien, Schatten, Abstände an einer Stelle. Dunkles Theme, bewusst nur ein Modus (kein Umschalter - privates Ein-Personen-Tool, kein Bedarf für einen halbfertigen Toggle). Akzentfarbe und die Kategorial-Palette im Spielervergleich-Chart sind dieselben dokumentierten Dark-Mode-Schritte aus der dataviz-Referenzpalette, gegen die dunkle Fläche validiert (`validate_palette.js --mode dark`) - App und Chart wirken dadurch als ein System statt als Einzelteile. Eigenes SVG-Logo (Mini-Spielfeld-Motiv, passend zum Taktik-Board) im Kopfbereich, kein externes Bild nötig. Responsiv bis Handy-Breite (getestet bei 375px): die durchgängigen flex-wrap-/auto-fit-Raster tragen das größtenteils von selbst (Filter, Taktik-Board, Karten und Balken-Raster stapeln sauber ohne horizontales Seiten-Scrollen); eigens angepasst wurden nur die Hauptnavigation (wird unter 600px zur vollbreiten, gestapelten Liste statt ungleichmäßig umzubrechen) und ein sichtbarer, dünner Scrollbalken für breite Tabellen (die vorher schon innerhalb ihres Wrappers scrollten, nur ohne erkennbaren Hinweis darauf auf einem Touch-Gerät).
...
- Spielerprofil: Detailansicht mit allen verfügbaren Feldern, in thematische Abschnitte gebündelt statt einer langen flachen Liste, in dieser Reihenfolge - **Basis** (Position, Nation, Geburtsdatum, Alter, Größe, Vertrag, Gehalt, Marktwert, Einsatzstatus), **Attribute** (alle glatten 1-20-Werte - die klassische FM-Attributskala), **Leistung** (Durchschnittsnote, Einsätze, echte Saison-Kennzahlen), **Weitere Felder** (alles Unklassifizierte, eingeklappt, damit keine Daten verschwinden). Alle Abschnitte nutzen dieselbe zweispaltige Optik (fällt auf schmalen Bildschirmen automatisch auf eine Spalte zurück, die Profilseite ist dafür auf 980px gewachsen). Prozentwerte (erkannt am Spaltennamen: "Anteil"/"Prozent"/"-quote"/"(%)") und 1-20-Attribute bekommen zusätzlich einen kompakten Balken - dieselbe Optik wie der Spielervergleich-Chart. Fehlt "Alter" ausnahmsweise im Export (in der Praxis so gut wie nie, da der Rest der App es voraussetzt), wird es ersatzweise aus dem Geburtsdatum berechnet - zum gesetzten Spieldatum, sonst zum heutigen Datum. Läuft eigenständig (wie Position/Vergleich), daher eine lokale, konservative Erkennung statt geratener FM-Kategorien (z. B. "Technik"/"Mental"/"Physis") - Länderspiel-Zähler wie "Lsp Eins" werden bewusst nicht als Attribut behandelt, obwohl sie zufällig im 1-20-Bereich liegen können
