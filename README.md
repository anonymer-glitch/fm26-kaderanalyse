...
- **Design:** ein Satz CSS-Variablen in `css/styles.css` (`:root`) statt verstreuter Hex-Werte - Farben, Radien, Schatten, Abstände an einer Stelle. Dunkles Theme, bewusst nur ein Modus (kein Umschalter - privates Ein-Personen-Tool, kein Bedarf für einen halbfertigen Toggle). Akzentfarbe und die Kategorial-Palette im Spielervergleich-Chart sind dieselben dokumentierten Dark-Mode-Schritte aus der dataviz-Referenzpalette, gegen die dunkle Fläche validiert (`validate_palette.js --mode dark`) - App und Chart wirken dadurch als ein System statt als Einzelteile. Eigenes SVG-Logo (Mini-Spielfeld-Motiv, passend zum Taktik-Board) im Kopfbereich, kein externes Bild nötig.
...
- Weitere Design-Politur: responsiveres Layout für schmale Bildschirme; Diagramme für Kaderweite Auswertungen statt nur Tabellen (z. B. Altersverteilung, Gehaltsstruktur, Vertragslaufzeiten) - das Balken-Chart-Muster aus dem Spielervergleich/Spielerprofil ließe sich dafür wiederverwenden
